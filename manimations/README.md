...
3. Skopiować pocięte pliki do `matura/<sheet-id>/media/zadN/krok-po-kroku/` pod nazwami `stepM.mp4` (nazwy lowercase, patrz CLAUDE.md). Zmiana z 2026-08-11: wcześniej leżały płasko jako `zadNrozw_stepM.mp4`.
4. Wygenerować rewersy: `tools/rewersy.sh matura/<sheet-id>/media/zadN/krok-po-kroku`. Robi to ffmpeg z gotowych plików, nie Manim — przycisk ◄ w odtwarzaczu odtwarza `stepMreverse.mp4`. Pułapki (m.in. konieczne przytrzymanie na końcu rewersu) opisuje [issues/krok-po-kroku-produkcja.md](../issues/krok-po-kroku-produkcja.md).
