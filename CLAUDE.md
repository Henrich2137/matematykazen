...
- `correctAnswerIndices` — (`multiSelect` only) array of correct indices; its length is the number of picks required. Clicks toggle a light-blue `.selected` state; once the required count is selected the picks are graded (green/red) and partial credit is granted (`maxScore · hits / required`, rounded). Deselecting re-opens the choice.
