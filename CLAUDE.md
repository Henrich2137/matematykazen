...
**Provenance warning:** exercises 1–20 match the original CKE sheet (texts for 7–20 verified against the `zadN/` image crops; answers recomputed by hand). Exercises **21–31 are consistent CKE-style stand-ins, not the originals** — the sheet PDF wasn't reachable from the sandbox. See todo.md ("DO ZROBIENIA — WAŻNE") before treating them as arkusz content. Also flagged there: the last frame of `zad2/zad2rozw_step6.mp4` shows 5⁻⁴ but the correct result is 5⁴ (answer A) — the video needs a Manim re-render; the data and step caption already carry the correction.
