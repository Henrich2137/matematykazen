...
**Wyjątek — praca nad wideo: `python3 -m http.server` NIE nadaje się.** Nie obsługuje żądań zakresowych (`Range`), a bez nich przeglądarka nie potrafi przewinąć filmu: `video.seekable` zostaje puste, a każde ustawienie `currentTime` cicho wraca do zera. Wygląda to jak błąd w kodzie odtwarzacza kroków i raz już nim nie było (2026-08-11). Do wszystkiego, co przewija film — kropki, rewersy — użyj `npx http-server` i sprawdź, że `curl -r 0-100` na plik wideo zwraca **206**, nie 200. Szczegóły w [issues/krok-po-kroku-produkcja.md](issues/krok-po-kroku-produkcja.md).
