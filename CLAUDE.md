...
A static Polish-language practice site for one specific exam sheet ("Egzamin maturalny z matematyki podstawowej grudzień 2024, próbna CKE"). No backend, no build system, no package manager. Four files drive everything:

- [index.html](index.html) — the landing page (hero + "Jak to działa" cards + demo-disclaimer section), pure static HTML styled by the `.landing-*` rules in style.css. Links to the exam sheet; the sheet's `#logo` links back here.
- [matematykazen.html](matematykazen.html) — the exam-sheet page around which all logic revolves: renders exercises from the data file, and handles all UI (answer buttons, hints, step-by-step solutions, the CKE formula sheet shown as an embedded PDF). Contains the hidden exercise `<template>` div and an inline `<script>` at the bottom that renders exercises and wires up all interactivity.
...
- [style.css](style.css) — all page styling (exam sheet + landing): minimalist, single centered column.
...
**Provenance:** exercises 1–30 match the original CKE sheet (MMAP-P0-100-2412; 7–20 verified against the `zadN/` image crops, 18–30 against the answer key supplied by the author; total = 50 pkt as in the official sheet). One reconstruction remains: the zad 29 data table was rebuilt to yield the key's results (mean 6,38 / median 6,5) — compare with the arkusz before trusting the counts (see todo.md). Also flagged there: the last frame of `zad2/zad2rozw_step6.mp4` shows 5⁻⁴ but the correct result is 5⁴ (answer A) — the video needs a Manim re-render; the data and step caption already carry the correction.

### Asset folder convention

Each exercise that needs images or video gets its own `zadN/` folder (matching the exercise number, not array index) referenced by relative path from the HTML's location, e.g. `zad6/zad6.png`, `zad6/zad6odp1.png` (odpowiedź = answer option image), `zad1/zad1rozw_step1.mp4` (rozwiązanie = solution). Keep filenames **lowercase** — `zad17.PNG` once broke on case-sensitive hosting and had to be renamed. Every image referenced from `exercises.js` currently exists on disk; the stand-in exercises 21–31 deliberately reference no images (when swapping in the originals, add their `zadN/` crops following this convention).
