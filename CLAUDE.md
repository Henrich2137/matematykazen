...
`.claude/settings.json` (tracked) enables **all five plugins** at scope `project`, so they travel with the repo: `superpowers` (14 skills: `brainstorming`, `systematic-debugging`, `writing-plans`, …), `frontend-design`, `chrome-devtools-mcp`, `github` and `cloudflare` (added 2026-09-03 through Cloudflare's official `agent-setup` procedure; 15 skills on Workers, `wrangler`, Durable Objects, web-perf. Its bundled MCP server is **deliberately unused**: `mcp.cloudflare.com` is not in the container's firewall allowlist and it would want an OAuth login to the account that serves matematykazen.pl. Skills only, deploys still go through the `wrangler` CLI). Since 2026-08-25 nothing is left at scope `local` (`.claude/settings.local.json` keeps only the sandbox settings). Skills only appear **after a Claude Code session restart**. Beyond the plugins, `.claude/skills/` holds two Manim skills copied from the MIT repo `adithya-s-k/manim_skill` (2026-08-25) - **house rules in `manimations/README.md` and `COLORS.md` win over them** - plus two of the project's own. **`projektowanie-rozwiazan`** (2026-08-26): the didactic layer, i.e. what the student is supposed to see and understand in a hint, a written solution, a step-by-step film or a widget, produced as a Polish design document and never as code; its `references/` carry the visual rules, the text rules, the Predict-Observe-Explain widget pattern and a list of typical student mistakes from the CKE reports. **`ogladanie-krokow`** (2026-08-29): the other end of the same pipe, i.e. checking a rendered film after the fact — you cannot see `.mp4`, only images, so [tools/klatki.sh](tools/klatki.sh) turns a film into frame sheets (`stany` / `film` / `styk`) and the skill says which mode to reach for, what to look at, what frames cannot show (smoothness, pacing — those stay with Henrich) and how not to burn context on images. `.claude/skills/README.md` says why. Install details, the marketplace-cache search trap and why `vendor/superpowers/` holds no plugin code: [issues/claude-code-pluginy.md](issues/claude-code-pluginy.md).
...
## Przypuszczenie to nie ustalenie (dodane 2026-08-23)

Cztery reguły wyciągnięte z sesji, w których coś prawdopodobnego zostało zapisane jak coś sprawdzonego. Każda kosztowała osobny render albo osobną paczkę pracy.

- **W notatce i w dokumentacji rozdzielaj „zmierzone" od „prawdopodobne".** „Nie ustalono" jest pełnoprawnym wynikiem i nie wolno go podmieniać na wyjaśnienie, które brzmi sensownie. Notatka, która zgaduje, wprowadzi w błąd za pół roku, kiedy nikt już nie pamięta, co było sprawdzone.
- **Cudze „nie wiem, jak to działa" w dokumentacji to zadanie do wykonania, nie kontekst do przyjęcia.** Zanim zaprojektujesz mechanizm, przeczytaj plik, który ten mechanizm ma zastąpić. Raz powstał w ten sposób cały plan cięcia scen na kroki, rozwiązujący problem, którego w tym zadaniu nie było.
- **Flagi i komendy, których nie uruchomiłeś, nie są jeszcze wiedzą.** Do kroku weryfikacyjnego dopisuj, **po czym poznać, że porównanie jest prawomocne** (przy wideo: zgodne wymiary i liczba klatek), inaczej krok potrafi „przejść" na niewłaściwym materiale i wyglądać na wykonany.
- **Hipotezę o przyczynie potwierdza pomiar, który mógłby ją obalić.** Przy różnicach wizualnych potrzebne są dwa dowody naraz: liczba (bo oko nie odróżni 0,9998 od 1,0) i obejrzenie najgorszego przypadku (bo liczba nie mówi, GDZIE jest różnica).
- **Ograniczenia środowiska sprawdzaj przed spisaniem planu, nie w połowie realizacji.** Jedno `touch .devcontainer/.probe` ujawniło montowanie tylko do odczytu i od razu ustawiło podział pracy host/kontener.
...
- **Nie pisz nazw liter słowami. Zamiast „iks", „iksa", „iksy" pisz wzór: \(x\)** (Henrich,
  2026-08-28). Dotyczy każdego tekstu dla ucznia: podpowiedzi, rozwiązania opisowego, opisów
  kroków pod filmem. „Dwa iksy bez jednego iksa" ma być zapisane rachunkiem
  (\(2x - 1x = 1x\)), a nie odczytane słowami. To samo z pozostałymi literami.
