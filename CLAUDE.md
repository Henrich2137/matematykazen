...
- `template.html` (root; replaces the old `matematykazen.html`) — the shared exam-sheet renderer, now the **single** page that renders *any* sheet: hidden exercise `<template>` + at the bottom a run of `<script src>` tags (`widgets/_helpers.js`, the nine `widgets/*.js` widget files, `widgets/_registry.js`, then the nine `app/*.js` files) that render exercises from a sheet's data file and wire up all interactivity. Which sheet is chosen by the `?arkusz=<id>` URL param (`<id>` = folder name under `matura/`); the per-sheet `matura/<id>/index.html` copies were removed.
- `matura/<sheet-id>/` (e.g. `matura/2024-grudzien/`, `matura/2026-maj/`) — one folder per exam sheet: its `exercises.json`, its `media/zadN/` assets (PNG images + Manim-produced MP4 solution videos; keep filenames **lowercase**) and its own exam/answer-key PDFs. All asset paths in `exercises.json` are **sheet-relative** and joined to the folder by `mediaPath()` in `app/state.js`.
...
Plus `vendor/katex/` — KaTeX vendored for fully offline math rendering (don't edit those files; to bump the version replace them from the npm tarball) — and `wybrane_wzory_matematyczne.pdf` (shared formula sheet shown in a floating panel; stays in the root).

- **`wybrane_wzory_matematyczne/`** — transcript of that PDF (created 2026-07-28), **for models, not served to users**: one Markdown file per CKE section (`01-…` … `16-…`) plus `INDEX.md`. Read it instead of the PDF whenever you need a formula — filling in `formulasPage`, checking a solution, writing hints. Start from `INDEX.md`: its "Skorowidz" maps exercise wording ("nierówność wykładnicza", "pole trapezu") to a formula ID and page, so you load one 300–800-token section rather than the whole sheet. Formulas use the **same KaTeX delimiters as `exercises.json`** (`\( … \)` / `\[ … \]`), so they paste straight into exercises — just remember JSON needs `\\`. Each formula carries its PDF page (printed = physical, no offset) and a coarse position (`góra`/`środek`/`dół`). `INDEX.md` has a **„Czego tu NIE MA"** section listing what the transcript does *not* carry (drawings/graphs → PDF pages 8, 11, 12, 15–28; section 17's trig value table → s. 34; front/back matter) — read it before concluding the formula sheet lacks something. Figures are rendered as **legends of symbols**, not descriptions of the drawing, and each affected section header says so with the PDF page to open. Verified 2026-07-28: every „•" bullet on pages 4–34 was listed from the PDF and matched one-to-one against transcript IDs (this caught one omission — `[8.10]` compound interest, since restored), all 795 formulas render in the vendored KaTeX, and every numerically checkable identity passes (26k random-value assertions). Each wired sheet's exam + answer-key PDFs (plus, where extracted, their `pdftotext -layout` `.txt` twins) now live in its own `matura/<id>/` folder — including not-yet-wired sheets like 2025-maj, which only has its PDFs so far, no `exercises.json`. `inne arkusze PDF/` is now empty but kept around for future extracts. The 2024-grudzień answers in `matura/2024-grudzien/exercises.json` were verified against the CKE key (2026-07-05).
