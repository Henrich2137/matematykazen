...
## Częściowo naprawione (2026-07-26, Opus)

`app/report.js` (link „zgłoś błąd") już używa `/Zadanie\s*([\d.]+)/i` z ucięciem kropki kończącej zdanie — przy okazji przebudowy formularza zgłoszeń. **Zostaje** `app/render.js`: sam regex tam był już poprawny, ale nadal do zrobienia są mylące `console.warn` z `index + 1` (linie ~141 i ~554).
