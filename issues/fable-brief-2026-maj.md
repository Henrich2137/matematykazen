...
Pilotaż na **czterech zadaniach** z `matura/2026-maj`. Dla każdego:

1. **podpowiedź** (`hint`) i **rozwiązanie opisowe** (`solutionText`) — arkusz nie ma ani jednego
2. **widżet interaktywny** — nowy plik w `widgets/`, jeśli zadanie się do tego nadaje
3. **scenariusz filmu Manima** — tekst sceny, **bez renderowania**
...
**Nie rozszerzaj zakresu sam**, nawet gdy zostanie budżet. Znalazłeś coś do poprawy
poza zakresem → wpis do `TODO.md`, nie poprawka.

## ⛔ STOP po pierwszym zadaniu

Zrób **jedno** zadanie w komplecie, potem **zatrzymaj się i oddaj do testów.**
Nie rób pozostałych trzech, dopóki Henrich nie potwierdzi. Chodzi o to, żeby
rozjazd stylu wyszedł po jednym zadaniu, a nie po czterech.
...
- **Czy scenariusz filmu da się obejrzeć w głowie** — czy kolejność kroków jest ta, którą
...
- **rozszerzania zakresu** poza cztery zadania
