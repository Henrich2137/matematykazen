...
4. Kliknięcie "zakończ egzamin" w karcie B wywołuje `finishExam()` w app/exam.js, która na starcie robi `if (!stan) return;` — `stan` jest `null` (bo klucz już usunięty przez kartę A), więc funkcja wychodzi natychmiast.
...
- Nasłuch zdarzenia `storage` w app/exam.js — gdy `KLUCZ_EGZAMINU` zniknie/zmieni się w innej karcie, karta B powinna zareagować (np. sama zakończyć/zresetować widok, albo pokazać komunikat "egzamin zakończony w innej karcie").
